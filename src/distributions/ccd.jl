...
log_density(ccd::CCD, cladified_tree::CladifiedTree) = sum(log_density(ccd, split) for split in values(cladified_tree.splits))
...
function point_estimate(ccd::CCD)
    ...
    return CladifiedTree(ccd.tip_names, ccd.root_clade, most_likely_splits, Dict{Clade, Float64}(), Dict{Clade, Float64}())
end
...
get_max_log_ccp(ccd::CCD, split::Split) = log_density(ccd, split) + get_max_log_ccp(ccd, split.clade1) + get_max_log_ccp(ccd, split.clade2)
@memoize get_max_log_ccp(ccd::CCD, clade::Clade) = maximum(get_max_log_ccp(ccd, split) for split in ccd.splits_per_clade[clade])
...
TractableTreeDistributions.entropy(ccd::CCD) = entropy(ccd, ccd.root_clade)
@memoize TractableTreeDistributions.entropy(ccd::CCD, clade::Clade) = sum(
...
TractableTreeDistributions.entropy(::CCD, ::Leaf) = 0.0
