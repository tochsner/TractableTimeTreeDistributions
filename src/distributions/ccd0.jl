struct CCD0 <: AbstractDistribution
    num_taxa::Int
    num_trees::Int
...
function CCD0(trees::Vector{Tree})
    ...
    return CCD0(num_taxa, num_trees, tip_names, root_clade, clades, splits, splits_per_clade, num_clade_occurrences, num_split_occurrences)
end

# get log probability

function log_density(ccd::CCD0, cladified_tree::CladifiedTree)
...
function most_likely_tree(ccd::CCD0)
...
function collect_most_likely_splits!(ccd::CCD0, current_clade::Clade, most_likely_splits::Dict{Clade,Split})
...
function collect_most_likely_splits!(ccd::CCD0, current_clade::Leaf, most_likely_splits::Dict{Clade,Split})
end

function get_max_log_ccp(ccd::CCD0, split::Split)
    log_density(ccd, split) + get_max_log_ccp(ccd, split.clade1) + get_max_log_ccp(ccd, split.clade2)
end

function get_max_log_ccp(ccd::CCD0, clade::Clade)
    maximum(get_max_log_ccp(ccd, split) for split in ccd.splits_per_clade[clade])
end

function get_max_log_ccp(ccd::CCD0, clade::Leaf)
    0.0
end

# sample tree

function sample_tree(ccd::CCD0)
...
function collect_sampled_splits!(ccd::CCD0, current_clade::Clade, sampled_splits::Dict{Clade,Split})
...
function collect_sampled_splits!(ccd::CCD0, current_clade::Leaf, sampled_splits::Dict{Clade,Split}) end
